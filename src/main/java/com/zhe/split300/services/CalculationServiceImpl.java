...
public class CalculationServiceImpl implements CalculationService {
    ...
    private Set<Calculation> convertPersonBalancesToCalculations(final Set<PersonBalance> personBalances, Evention evention) {
        ...
        while (true) {
            ...
            if (balancesMap.get(minBalancePerson).compareTo(BigDecimal.ZERO) >= 0 || balancesMap.get(maxBalancePerson).compareTo(BigDecimal.ZERO) <= 0) {
                break;
            }
            BigDecimal calculationValue = balancesMap.get(maxBalancePerson).min(balancesMap.get(minBalancePerson).abs());
